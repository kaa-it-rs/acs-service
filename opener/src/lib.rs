...
pub async fn run(
    ...
    let res = run_internal(address, port, serial_number, model, login, password).await;
    if let Err(e) = res {
...
async fn run_internal(
    ...
    let client = ws_client::WSClient::new(address, port, serial_number, model, login, password);
